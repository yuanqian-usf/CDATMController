package com.circulardollar.cdatm.play.network;
...
public class __NetworkClientV2 implements INetworkClientV2 {
    ...
    private final IATMRemoteValidator validator;

    private __NetworkClientV2(Map<ILoginRecord, IAccountsRecord> accountsRecordTable, IATMRemoteValidator validator) {
    ...
    @Override public CompletableFuture<RemoteResponse<IAuthRecord>> verifyPin(
        RemoteRequest<ILoginRecord> loginRequest) {
    ...
    @Override public CompletableFuture<RemoteResponse<IAccountsRecord>> getAccounts(
        RemoteRequest<IRequestWithToken> tokenRequest) {
        ...
        if (tokenRequest == null || tokenRequest.getBody() == null || tokenRequest.getBody()
            .getTokenId() == null || tokenRequest.getBody().getTokenId().isEmpty()) {
        ...
        if (!authTokenLastLoginTimeStampTable.containsKey(tokenRequest.getBody().getTokenId()) || isExpiredToken(
            authTokenLastLoginTimeStampTable.get(tokenRequest.getBody().getTokenId()))) {
            return CompletableFuture.completedFuture(builder.setError(ErrorRecord
                .of(INetworkClientV2.class,
                    Collections.singletonList(Errors.INVALID_TOKEN.getValue()))).build());
        }
        String cardNumber = authTokenCardNumberTable.get(tokenRequest.getBody().getTokenId());
    ...
    @Override public CompletableFuture<RemoteResponse<IDepositRecord>> deposit(
        RemoteRequest<IDepositRecordRequest> depositRequest) {
    ...
    @Override public CompletableFuture<RemoteResponse<IWithdrawRecord>> withdraw(
        RemoteRequest<IWithdrawRecordRequest> withdrawRequest) {
    ...
    public static class Builder extends IBuilder {
        ...
        public INetworkClientV2 build() {
            return new __NetworkClientV2(accountsRecordTable, validator);
        }
    }
}
